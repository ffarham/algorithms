/**
 * Farham Ahmad
 * Data Structure: Hash-ring (see
 * reference 1).
 * Application: Consistent hashing in load balancer.
 * 
 * To map an object on the hash-ring, we use the built in hash function on
 * strings. The hash-ring will be implemented as a binary search tree using 
 * the set class, similar to ordered set implementation of reference 3.
 * 
 * Each server node will be replicated 200 times following the results of
 * reference 4; suggesting that 'one to two hundered replicas achieves 
 * an acceptable balance'. When distributing 10,000 keys to 10 servers with 
...
public:
  ...
  void insert(int id) {
    ...
    for (int i {0}; i < replication_factor; i++)
    {
      const std::string value {
        "server " + std::to_string(id) + " version " + std::to_string(i)
      };
      const std::size_t hash_value {std::hash<std::string>{}(value)};

      // NOTE: any collisions are over-written on the hash-ring
  ...
  int assign(int id)
  {
    ...
    std::size_t hashed_server {
      (pos == hash_ring.end()) ? *hash_ring.begin() : *pos
    };
...
int main() {
  ...
  for (int server {0}; server < servers; server++){
    std::cout << "server: " << server << " allocations: " 
    << server_allocations[server] << '\n'; 
  }

  const double sum = std::accumulate(server_allocations.begin(), server_allocations.end(), 0);
  const double mean {sum / server_allocations.size()};

  double accum {0.0};
  std::for_each (std::begin(server_allocations), std::end(server_allocations),
    [&](const double d) {
      accum += (d - mean) * (d - mean);
    }
  );
